...
#if ENABLE_PROFILING
    printf("Starting SW Movement\n");
    profileCiResetCounters();
    profileCiEnableCounters();
#endif
      movement_result = movmentDetectSW(sobel, previous_sobel, camParams.nrOfPixelsPerLine, camParams.nrOfLinesPerImage);
  #if ENABLE_PROFILING
    profileCiDisableCounters();
    profileCiPrintCounters();
  #endif
      if(movement_result){
        printf("Movement detected in software! %d times\n", ++movement_detected_counter);
        movement_detected_counter++;
      }
